...
from pipelines.instruction_tuning.model.model_utils import (
    make_model_lora,
    get_peft_state,
...
class ModelArguments:
    ...
    modality_builder_type: str = field(default="small")
    model_lora_path: Optional[str] = field(default=None)

class LMMTrainer(Trainer):
    def _save_checkpoint(self, model, trial, metrics=None):
        if self.args.save_strategy == "epochs":
            epoch = math.ceil(self.state.epoch)
            checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-epochs-{epoch}"
            ...
            super(LMMTrainer, self)._save_checkpoint(model, trial) 
            self.args.output_dir = original_output_dir
        elif self.args.save_strategy == "steps":
            checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-steps-{self.state.global_step}"
            run_dir = self._get_output_dir(trial=trial)
            output_dir = os.path.join(run_dir, checkpoint_folder)
            self._save_extras(output_dir)
            if metrics is not None:
    ...
    def _save_metrics(self, output_dir, metrics):
...
def train_for_modalities(
    ...
    model.config.modality_builder_type = model_args.modality_builder_type
    ...
    for name, param in model.named_parameters():
        if "audio_whisper_lmm_projector" in name:
            param.data = param.data.to(torch.bfloat16)
            if param.grad is not None:
                param.grad.data = param.grad.data.to(torch.bfloat16)
